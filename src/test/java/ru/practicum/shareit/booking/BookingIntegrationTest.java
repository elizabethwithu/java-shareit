...
public class BookingIntegrationTest {
    ...
    void getAllBookings_Current() {
        List<BookingOutputDto> list4 = bookingService.findAllUsersBooking(booker.getId(), "CURRENT", 0, 20);
        Assertions.assertThat(list4).isNotEmpty().hasSize(2);
    }

    @Test
    void getAllBookings_Rejected() {
        List<BookingOutputDto> list5 = bookingService.findAllUsersBooking(booker.getId(), "REJECTED", 0, 20);
        Assertions.assertThat(list5).isNotEmpty().hasSize(1);
    }

    @Test
    void getAllBookings_Waiting() {
        List<BookingOutputDto> list6 = bookingService.findAllUsersBooking(booker.getId(), "WAITING", 0, 20);
        Assertions.assertThat(list6).isNotEmpty().hasSize(1);
    }

    @Test
    void getAllBookingsForOwner_All() {
        List<BookingOutputDto> list1 = bookingService.findAllBookingsForItems(owner.getId(), "ALL", 0, 20);
        Assertions.assertThat(list1).isNotEmpty().hasSize(4);
    }

    @Test
    void getAllBookingsForOwner_Past() {
        List<BookingOutputDto> list2 = bookingService.findAllBookingsForItems(owner.getId(), "PAST", 0, 20);
        Assertions.assertThat(list2).isNotEmpty().hasSize(1);
    }

    @Test
    void getAllBookingsForOwner_Future() {
        List<BookingOutputDto> list3 = bookingService.findAllBookingsForItems(owner.getId(), "FUTURE", 0, 20);
        Assertions.assertThat(list3).isNotEmpty().hasSize(1);
    }

    @Test
    void getAllBookingsForOwner_Current() {
        List<BookingOutputDto> list4 = bookingService.findAllBookingsForItems(owner.getId(), "CURRENT", 0, 20);
        Assertions.assertThat(list4).isNotEmpty().hasSize(2);
    }

    @Test
    void getAllBookingsForOwner_Rejected() {
        List<BookingOutputDto> list5 = bookingService.findAllBookingsForItems(owner.getId(), "REJECTED", 0, 20);
        Assertions.assertThat(list5).isNotEmpty().hasSize(1);
    }

    @Test
    void getAllBookingsForOwner_Waiting() {
        List<BookingOutputDto> list6 = bookingService.findAllBookingsForItems(owner.getId(), "WAITING", 0, 20);
        Assertions.assertThat(list6).isNotEmpty().hasSize(1);
    }
}
